  ...
  const renderContentBlock = (block: ContentBlock, index: number, isStreaming?: boolean, isLast?: boolean) => {
    if (block.type === 'text') {
      return (
        <div 
          key={`text-${index}`}
          className="message-bubble-assistant leading-relaxed break-words"
        >
  ...
  const renderAssistantContent = (message: Message) => {
    ...
    return (
      <>
        ...
        {message.content && (
          <div className="message-bubble-assistant leading-relaxed break-words">
            <div className="chat-markdown">
    ...
    <div className="flex flex-col gap-5 max-w-3xl mx-auto w-full">
      {messages.map(message => (
        ...
        >
          {/* 头像 */}
          <div 
            className="w-7 h-7 flex items-center justify-center flex-shrink-0 rounded-lg self-start"
            style={{
          ...
          <div 
            className={`flex flex-col gap-2 max-w-[85%] ${message.role === 'user' ? 'items-end' : ''}`}
          >
            {/* 模型标签 */}
            ...
            {/* 用户消息 */}
            {message.role === 'user' && (
              <div className="message-bubble-user leading-relaxed break-words text-[14px]">
                {message.content}
              </div>
            )}
